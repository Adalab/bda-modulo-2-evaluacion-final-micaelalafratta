...
FROM
    actor
WHERE
    actor_id BETWEEN 10 AND 20; -- Between incluye los dos valores.
...
-- Query final: 
SELECT 
    rating, COUNT(film_id) AS quantity_films
FROM
    film
GROUP BY rating; 
...
SELECT 
    ...
    c.first_name,
    c.last_name,
    COUNT(r.rental_id) AS quantity_films_rented
FROM
    customer AS c
        LEFT JOIN
    rental AS r ON c.customer_id = r.customer_id
GROUP BY c.customer_id
ORDER BY quantity_films_rented;
...
SELECT 
    c.name AS category,
    COUNT(r.rental_id) AS rented_films_per_category
FROM
    rental AS r
        INNER JOIN
    inventory AS i ON r.inventory_id = i.inventory_id
...
-- Query final:
SELECT 
    rating AS film_rating, ROUND(AVG(length), 0) AS AVG_length
FROM
    film
GROUP BY rating; 
...
WHERE
    description LIKE '%cat%'
        OR description LIKE '%dog%'; -- se debe indicar dos veces description, si no, no es una condición completa.
...
FROM
    film
WHERE
    description REGEXP 'cat|dog';   -- Con Regex
...
SELECT 
    a.actor_id, fa.actor_id, fa.film_id
FROM
    actor AS a
        LEFT JOIN
...
FROM
    ...
    film_actor AS fa ON a.actor_id = fa.actor_id
WHERE
    fa.film_id IS NULL;     -- No devuelve resultados. No hay actores o actrices que no aparezcan en ningúna película en la tabla film_actor. 
...
-- Query final: 
SELECT 
    f.title AS film_title, c.name AS category
FROM
    film AS f
        INNER JOIN
    film_category AS fc ON f.film_id = fc.film_id
...
FROM
 ...
 -- Query final: 
SELECT 
    a.first_name, a.last_name, COUNT(fa.film_id) AS total_films
FROM
    actor AS a
        INNER JOIN
    film_actor AS fa ON a.actor_id = fa.actor_id
...
FROM
    film
WHERE
    rating = 'R' AND length > 120;  -- No se incluyen las películas de 120. 
...
SELECT 
    c.name AS category, ROUND(AVG(f.length), 0) AS AVG_length
FROM
    film AS f
        INNER JOIN
    film_category AS fc ON f.film_id = fc.film_id
...
ORDER BY quantity_films ASC;  -- ordenamos por cantidad de películas para comprobar el resultado y facilitar su lectura.
...
SELECT 
    f.title AS film_title, r.rental_id
FROM
    film AS f
        INNER JOIN
    inventory AS i ON f.film_id = i.film_id
...
-- Query final opción 1:
SELECT 
    a.first_name, a.last_name
FROM
    actor AS a
WHERE
    a.actor_id NOT IN (SELECT DISTINCT
            fa.actor_id
        FROM
            film_actor AS fa
                INNER JOIN
            film_category AS fc ON fa.film_id = fc.film_id
                INNER JOIN
            category AS c ON fc.category_id = c.category_id
        WHERE
            c.name = 'Horror')
...
-- Query final: 
SELECT 
    f.title, f.length, c.name AS category_name
FROM
    film AS f
        INNER JOIN
    film_category AS fc ON f.film_id = fc.film_id
    ...
    category AS c ON fc.category_id = c.category_id
WHERE
    c.name = 'Comedy' AND f.length > 180;   -- No es necesario agrupar. Uso de WHERE para filtrar por filas individuales.
